...
session_parameters = bootornado.utils.storage({
    'cookie_name': 'tornado_session_id',
    'cookie_domain': None,
    'cookie_path' : None,
...
class SessionExpired(tornado.web.HTTPError): 
    def __init__(self, message):
        tornado.web.HTTPError.__init__(self, '200 OK', {}, data=message)
...
class Session(object):
    ...
    def __init__(self, store, initializer=None):
        self.store = store
        self._initializer = initializer
        self._last_cleanup_time = 0
        self._config = bootornado.utils.storage(session_parameters)
    ...
    def _load(self):
        ...
        self.ip = self.handler.request.remote_ip
    ...
    def _validate_ip(self):
        ...
        if self.session_id and self.get('ip', None) != self.handler.request.remote_ip:
    ...
    def _generate_session_id(self):
        ...
        while True:
            ...
            session_id = sha1("%s%s%s%s" %(rand, now, bootornado.utils.safestr(self.handler.request.remote_ip), secret_key))
    ...
    def _valid_session_id(self, session_id):
        rx = bootornado.utils.re_compile('^[0-9a-fA-F]+$')
